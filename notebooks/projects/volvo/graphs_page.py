...
def create_graphs(hallazgos_vref, betas_GDL, betas_MDE):
    ...
    colonias_guadalajara = ["Providencia", "Miramar", "Colinas"]
    colonias_medellin = ["Moravia", "Floresta", "Aguacatala"]
...
def extraer_fila_por_concepto(df, concepto_buscar):
    ...
    # print(f"Buscando concepto: '{concepto_limpio}'")
    ...
    for col_idx in [0, 1]:
        if col_idx < df.shape[1]:
            ...
            # Debug: Mostrar valores únicos de la columna
            # if col_idx == 1:  # Solo para la segunda columna donde están los conceptos
                # print(f"Valores únicos en columna {col_idx}:")
                # print(col_data.unique()[:10])  # Mostrar solo los primeros 10

            mask = col_data.str.contains(concepto_limpio, case=False, na=False, regex=False)

            if mask.any():
                fila_idx = mask.idxmax()
                fila_encontrada = df.iloc[fila_idx, :]
                # print(f"Fila encontrada para '{concepto_limpio}': {fila_encontrada.iloc[1]} - Valores: {fila_encontrada.iloc[1:7].values}")
                return fila_encontrada

    # print(f"No se encontró el concepto: '{concepto_limpio}'")
    return None
...
def obtener_valor_seguro(fila, indice_zona):
    ...
    try:
        if fila is None or indice_zona >= len(fila):
            print(f"Error: fila es None o índice {indice_zona} fuera de rango (len: {len(fila) if fila is not None else 'None'})")
            return 0

        valor_raw = fila.iloc[indice_zona]
        # print(f"Valor raw en índice {indice_zona}: '{valor_raw}' (tipo: {type(valor_raw)})")

        valor_convertido = convertir_a_float_seguro(pd.Series([valor_raw])).iloc[0]
        # print(f"Valor convertido: {valor_convertido}")

        return valor_convertido

    except (IndexError, ValueError, TypeError) as e:
        print(f"Error en obtener_valor_seguro: {e}")
        return 0
...
def grafico_radar(betas_df, zona_seleccionada, colores_dict):
    """Crea gráfico radar para una zona específica con etiquetas personalizadas"""

    # Diccionario de traducción
    traducciones = {
        "land_use": "Uso de suelo",
        "den_pop": "Densidad poblacional",
        "sidewalk": "Banquetas",
        "grade_abs": "Pendientes",
        "ind_ndvi": "Índice de vegetación",
        "den_inter": "Intersecciones"
    }

    # Obtener categorías originales y traducirlas si corresponde
    categorias_originales = betas_df['Variable'].tolist()
    categorias_traducidas = [traducciones.get(cat, cat) for cat in categorias_originales]

    # Obtener los valores para la zona seleccionada
