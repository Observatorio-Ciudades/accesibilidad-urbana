...
if __name__ == "__main__":
	...
	for city, data in  areas.items():
		try:
			...
			area = aup.load_polygon(city) #load the gdf with the polygon to use
			polygon = area['geometry'][0]
			G = aup.download_graph(polygon,city,network_type='all_private',save=True) #load the graph
			for amenity_name in amenities:
				try:
					...
					gdf = aup.load_denue(amenity_name) #Load the Denue
					gdf_f = gpd.clip(gdf,area)
					logging.info(f'{city} {amenity_name} data not in system, starting to look for nearest nodes')
					gdf_f = aup.find_nearest(G,gdf_f,amenity_name) #find nearest nodes to seeds
				...
				nodes = aup.calculate_distance_nearest_poi(gdf_f,G,amenity_name, city) #run the calculations and return the final dataframe
