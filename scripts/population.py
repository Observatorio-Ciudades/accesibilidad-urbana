...
def main(city, gdf_pop, columns):
    ...
    G = aup.download_graph(
        gdf_area, city, network_type='all_private', save=False)
    ...
    hex_bins = pd.merge(hex_bins, hex_temp, right_index=True,
                        left_on=f'hex_id_{resolution}', how='left').fillna(0)
    hex_bins.to_file(
        f"../data/processed/{city}_hex_bins.geojson", driver='GeoJSON')
