...
def main(city, res, mun_gdf, ageb_gdf, save=False):
    ...
    pop_join = ageb_gdf.loc[ageb_gdf.cvegeo_ageb.isin(list(ageb_join.cvegeo_ageb.unique()))].pobtot.sum()
...
if __name__ == "__main__":
    ...
    for city in city_names:
        ...
        table = 'censo_inegi_20_ageb'
        ageb_gdf = gpd.GeoDataFrame()
        for cvegeo in mun_gdf.CVEGEO.unique():
            query = f"SELECT cvegeo_ageb,pobtot,geometry FROM {schema}.{table} WHERE \"cvegeo_ageb\" LIKE \'{cvegeo}%%\'"
